<template>
  <div>
    ...
    <div v-if="dashboardData" class="grid grid-cols-1 lg:grid-cols-2 justify-center lg:ml-[3rem] sm:ml-0 ml-6 cursor-pointer">
      <div
      ...
      >
        ...
        <div class="text-center mt-4">
          <h3 class="sm:w-[350px] mx-auto absolute inset-0 top-[2rem] font-semibold text-[18px] text-white sm:px-0 px-10">
            {{ feedback.job_title }}
          </h3>
          <p class="absolute inset-0 sm:top-[100px] top-[160px] font-thin text-gray-100">
            {{ feedback.user_detail.fname }}
            {{ feedback.user_detail.lname }}
          </p>
          <p class="absolute inset-0 sm:top-[120px] top-[180px] text-gray-400 pt-1">{{ feedback.user_detail.email }}</p>
          <p class="absolute inset-0 sm:top-[150px] top-[230px] text-white font-medium sm:pt-2 pt-2 bg-pink-500 h-10">{{ feedback.feedback_text }}</p>
        </div>
      </div>
    </div>
    <!-- <div
